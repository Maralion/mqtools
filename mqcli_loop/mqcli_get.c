...
int main(int argc, char **argv)
{
	...
	MQGMO   gopt = { MQGMO_DEFAULT };   // GET message options

	MQOD    rod = { MQOD_DEFAULT };     // Ref object descriptor
	MQMD    rmd = { MQMD_DEFAULT };     // Ref message descriptor
	MQGMO   rgopt = { MQGMO_DEFAULT };  // Ref GET message options
	...
	for (;;)
	{
		...
		MQOPEN(hconn, &od, MQOO_INPUT_SHARED | MQOO_FAIL_IF_QUIESCING, &hobj, &cc, &re);
		...
		for(;;)
		{
			memcpy(&gopt,&rgopt,sizeof(gopt));
			gopt.Options = MQGMO_WAIT | MQGMO_SYNCPOINT | MQGMO_FAIL_IF_QUIESCING;
			gopt.WaitInterval=MQWI_UNLIMITED;

			memcpy(&md,&rmd,sizeof(md));

			MQGET(hconn, hobj, &md, &gopt, sizeof(msgbuf), msgbuf, &msglen, &cc, &re);
			if (re != MQRC_NONE)
			{
				pgm_err("MQGET() cc=%d re=%d\n", cc, re);
				break;
			}
